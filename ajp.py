    ...
    def pack_headers(self):
        ...
        for h_name in self.request_headers:
            if h_name.startswith("SC_REQ"):
                code = AjpForwardRequest.COMMON_HEADERS.index(h_name)
...
class AjpResponse(object):
    ...
    # send headers codes
    COMMON_SEND_HEADERS = ["Content-Type", "Content-Language", "Content-Length", "Date", "Last-Modified", "Location", "Set-Cookie", "Set-Cookie2", "Servlet-Engine", "Status", "WWW-Authenticate"]
    ...
    def parse_send_body_chunck(self, stream):
        return
...
class AjpDataPacket(object):
    pass

import socket

def test_password(user, password, target_host, target_port):
    ...
    fr.num_headers = len(fr.request_headers)
    fr.is_ssl = False

    fr.attributes = [
            {"name": "req_attribute", "value": ("AJP_LOCAL_ADDR", "127.0.0.1")},
    ...
    s.sendall(fr.serialize())

    for i in range(2):
        r = AjpResponse()
        r.parse(s.recv(10000))
        pprint(vars(r))
...
if __name__ == "__main__":
    test_password("tomcat", "tomcat", "127.0.0.1", 8009)
#    fr = AjpForwardRequest(AjpForwardRequest.SERVER_TO_CONTAINER)
#    fr.method = AjpForwardRequest.GET
#    fr.protocol = "TEST"
#    fr.req_uri = "http://test/manager/html"
...
#    fr.is_ssl = False
#    fr.request_headers = {
#        "SC_REQ_COOKIE": "GIMME COOKIE",
#        "my custom header": "custom value"
#     }
#
#    fr.attributes = {
#        "auth_type": "Basic",
#        "req_attribute": ("custom_attribute", "bla")
#    }
#
#    print repr(fr.serialize())
#
#    # test the initial request
#    packets = rdpcap("samples/authent.pcap")
#    authent_packet = packets[8]
#    authent_payload = authent_packet[3].load
#    
#    print repr(authent_payload)
#
#    fr2 = AjpForwardRequest()
#    fr2.parse(authent_payload)
#    pprint (vars(fr2))
#
#    # and its response
#    authent_response_packet = packets[20]
#    authent_response_payload = authent_response_packet[3].load
#
#    print repr(authent_response_payload)
#    fr3 = AjpResponse()
#    fr3.parse(authent_response_payload)
#    pprint (vars(fr3))
